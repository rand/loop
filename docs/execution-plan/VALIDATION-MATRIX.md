...
| VG-LA-001 | `loop-agent` seam-critical compatibility subset | `LOOP_MIN_AVAILABLE_MIB=3072 /Users/rand/src/loop/scripts/safe_run.sh bash -lc 'cd /Users/rand/src/loop-agent && uv run pytest -q tests/test_router.py tests/test_trajectory.py tests/test_set_backend_propagation.py tests/test_sensitivity_wiring.py'` | All seam-critical tests pass | `.../VG-LA-001.txt` |
| VG-LA-002 | `loop-agent` full-suite health snapshot (advisory unless promoted by D-014) | `LOOP_MIN_AVAILABLE_MIB=3072 /Users/rand/src/loop/scripts/safe_run.sh bash -lc 'cd /Users/rand/src/loop-agent && uv run pytest -q'` | Snapshot captured and failures triaged in evidence; promotion to blocking requires D-014 criteria | `.../VG-LA-002.txt` |
